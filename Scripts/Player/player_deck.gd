...
func _ready() -> void:
	...
	#await Global.update_phase()
	#$"../Labels".update_phase_label()
...
func checkDeckType():
	if Global.player_deck_chosen == "FireDeck" or player_deck_chosen == "FireDeck": 
...
func draw_card():
	...
	# Local deck setting for testing.
	#var card_image = load(card_database.fireDeckDetails[card_drawn][6])
	#await new_card.change_card_image(card_image)	
	...
	new_card.energy = card_database.fireDeckDetails[card_drawn][1]
	
	# Set abilities of the card
	new_card.ability1 = str(card_database.fireDeckDetails[card_drawn][2])
	new_card.ability2 = str(card_database.fireDeckDetails[card_drawn][3])
	new_card.ability3 = str(card_database.fireDeckDetails[card_drawn][4])
