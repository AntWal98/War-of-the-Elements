...
const DEFAULT_CARD_SCALE: float = 1.0
const BIG_CARD_SCALE: float = 1.15
...
func finish_drag():
	...
	if card_slot_found and not card_slot_found.card_in_slot:
		# If card is a monster and placed in monster card slot
		if card_being_dragged.card_type == card_slot_found.card_slot_type:
			if !monster_card_played:
				monster_card_played = true
				...
				return
	player_hand_reference.add_card_to_hand(card_being_dragged, Global.DEFAULT_CARD_MOVE_SPEED)
	card_being_dragged = null
